	...
	if (l('tooltipAnchor').style.display != 'none' && l('CMTooltipArea') != null) {
		...
		if (Addon.tooltipType == 'b') {
			...
			if (Addon.Tooltip == 1 && Game.buyMode == 1) {
				l('CMTooltipBorder').className = Addon.colorTextPre + Addon[target][Addon.tooltipName].color;
				l('CMTooltipPP').textContent = Beautify(Addon[target][Addon.tooltipName].pp, 2);
				l('CMTooltipPP').className = Addon.colorTextPre + Addon[target][Addon.tooltipName].color;
			}
		}
		else { // Upgrades
