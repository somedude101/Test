...
Addon.RemakeBuildingsOtherPP = function(amount, target) {
	for (var i in Addon[target]) {
		//Addon[target][i].pp = Addon[target][i].price / Addon[target][i].bonus;
		Addon[target][i].pp = (Math.max(Addon[target][i].price - (Game.cookies + Addon.GetWrinkConfigBank()), 0) / Game.cookiesPs) + (Addon[target][i].price / Addon[target][i].bonus);
		var color = '';
		if (Addon[target][i].pp <= 0 || Addon[target][i].pp == 'Infinity') color = Addon.colorGray;
		else if (Addon[target][i].pp < Addon.min) color = Addon.colorBlue;
		else if (Addon[target][i].pp == Addon.min) color = Addon.colorGreen;
		else if (Addon[target][i].pp == Addon.max) color = Addon.colorRed;
		else if (Addon[target][i].pp > Addon.max) color = Addon.colorPurple;
		else if (Addon[target][i].pp > Addon.mid) color = Addon.colorOrange;
		else color = Addon.colorYellow;
		Addon[target][i].color = color;
	}
}
...
Addon.SaveConfig = function(config) {
	localStorage.setItem(AddonPrefix, JSON.stringify(config));
}

Addon.LoadConfig = function() {
	if (localStorage.getItem(AddonPrefix) != null) {
		Addon = JSON.parse(localStorage.getItem(AddonPrefix));
		
		// Check values
		var mod = false;
		for (var i in AddonDefault) {
			if (typeof Addon[i] === 'undefined') {
				mod = true;
				Addon[i] = AddonDefault[i];
			}
			else if (i != 'StatsPref' && i != 'Colors') {
				if (i.indexOf('SoundURL') == -1) {
					if (!(Addon[i] > -1 && Addon[i] < Addon[i].label.length)) {
						mod = true;
						Addon[i] = AddonDefault[i];
					}
				}
				else {  // Sound URLs
					if (typeof Addon[i] != 'string') {
						mod = true;
						Addon[i] = AddonDefault[i];
					}
				}
				...
				for (var j in AddonDefault.StatsPref) {
					if (typeof Addon[i][j] === 'undefined' || !(Addon[i][j] > -1 && Addon[i][j] < 2)) {
						mod = true;
						Addon[i][j] = AddonDefault[i][j];
					}
				}
			}
			else { // Colors
				for (var j in AddonDefault.Colors) {
					if (typeof Addon[i][j] === 'undefined' || typeof Addon[i][j] != 'string') {
						mod = true;
						Addon[i][j] = AddonDefault[i][j];
					}
				}
		...
		if (mod) Addon.SaveConfig(Addon);
		Addon.Loop(); // Do loop once
		for (var i in AddonDefault) {
			if (i != 'StatsPref' && typeof Addon[i].func !== 'undefined') {
				Addon[i].func();
			}
		}
	}
...
Addon.ToggleConfig = function(config) {
	...
	if (Addon[config].toggle) {
		if (Addon[config] == 0) {
			l(AddonPrefix + config).className = 'option off';
		}
		else {
			l(AddonPrefix + config).className = 'option';
		}
	}
}
...
Addon.ToggleConfigUp = function(config) {
	Addon[config]++;
	if (Addon[config] == Addon[config].label.length) {
		Addon[config] = 0;
	}
	if (typeof Addon[config].func !== 'undefined') {
		Addon[config].func();
	}
	l(AddonPrefix + config).innerHTML = Addon.GetConfigDisplay(config);
	Addon.SaveConfig(Addon);
}

Addon.ToggleConfigDown = function(config) {
	Addon[config]--;
	if (Addon[config] < 0) {
		Addon[config] = Addon[config].label.length - 1;
	}
	if (typeof Addon[config].func !== 'undefined') {
		Addon[config].func();
	}
...
Addon.GetConfigDisplay = function(config) {
	return Addon[config].label[Addon[config]];
}
...
Addon.CreateTimerBar = function() {
	...
	Addon.TimerBar.id = 'AddonTimerBar';
	...
	Addon.TimerBarGC.id = 'AddonTimerBarGC';
	Addon.TimerBarGC.style.height = '12px';
	Addon.TimerBarGC.style.margin = '0px 10px';
	Addon.TimerBarGC.style.position = 'relative';
	Addon.TimerBarGC.appendChild(bar('Next Cookie', [{id: 'AddonTimerBarGAddoninBar', color: Addon.colorGray}, {id: 'AddonTimerBarGCBar', color: Addon.colorPurple}], 'AddonTimerBarGCTime'));
	Addon.TimerBar.appendChild(Addon.TimerBarGC);
	
	Addon.TimerBarRen = document.createElement('div');
	Addon.TimerBarRen.id = 'AddonTimerBarRen';
	Addon.TimerBarRen.style.height = '12px';
	Addon.TimerBarRen.style.margin = '0px 10px';
	Addon.TimerBarRen.style.position = 'relative';
	Addon.TimerBarRen.appendChild(bar('Next Reindeer', [{id: 'AddonTimerBarRenMinBar', color: Addon.colorGray}, {id: 'AddonTimerBarRenBar', color: Addon.colorOrange}], 'AddonTimerBarRenTime'));
	Addon.TimerBar.appendChild(Addon.TimerBarRen);
	
	Addon.TimerBarBuff1 = document.createElement('div');
	Addon.TimerBarBuff1.id = 'AddonTimerBarBuff1';
	...
	Addon.TimerBarBuff1.appendChild(bar('', [{id: 'AddonTimerBarBuff1Bar'}], 'AddonTimerBarBuff1Time'));
	Addon.TimerBarBuff1.firstChild.firstChild.id = 'AddonTimerBarBuff1Type';
	Addon.TimerBar.appendChild(Addon.TimerBarBuff1);
	
	Addon.TimerBarBuff2 = document.createElement('div');
	Addon.TimerBarBuff2.id = 'AddonTimerBarBuff2';
	...
	Addon.TimerBarBuff2.appendChild(bar('', [{id: 'AddonTimerBarBuff2Bar'}], 'AddonTimerBarBuff2Time'));
	Addon.TimerBarBuff2.firstChild.firstChild.id = 'AddonTimerBarBuff2Type';
...
Addon.UpdateTimerBar = function() {
	if (Addon.TimerBar == 1) {
		...
		if (Game.shimmerTypes['golden'].spawned == 0 && !Game.Has('Golden switch [off]')) {
			...
			l('AddonTimerBarGAddoninBar').style.width = Math.round(Math.max(0, Game.shimmerTypes['golden'].minTime - Game.shimmerTypes['golden'].time) * maxWidth / Game.shimmerTypes['golden'].maxTime) + 'px';
			if (Game.shimmerTypes['golden'].minTime == Game.shimmerTypes['golden'].maxTime) {
				l('AddonTimerBarGAddoninBar').style.borderTopRightRadius = '10px';
				l('AddonTimerBarGAddoninBar').style.borderBottomRightRadius = '10px';
			}
			else {
				l('AddonTimerBarGAddoninBar').style.borderTopRightRadius = '';
				l('AddonTimerBarGAddoninBar').style.borderBottomRightRadius = '';
			}
			l('AddonTimerBarGCBar').style.width = Math.round(Math.min(Game.shimmerTypes['golden'].maxTime - Game.shimmerTypes['golden'].minTime, Game.shimmerTypes['golden'].maxTime - Game.shimmerTypes['golden'].time) * maxWidth / Game.shimmerTypes['golden'].maxTime) + 'px';
			l('AddonTimerBarGCTime').textContent = Math.ceil((Game.shimmerTypes['golden'].maxTime - Game.shimmerTypes['golden'].time) / Game.fps);
			count++;
		}
		else {
		...
		if (Game.season == 'christmas' && Game.shimmerTypes['reindeer'].spawned == 0) {
			...
			l('AddonTimerBarRenMinBar').style.width = Math.round(Math.max(0, Game.shimmerTypes['reindeer'].minTime - Game.shimmerTypes['reindeer'].time) * maxWidth / Game.shimmerTypes['reindeer'].maxTime) + 'px';
			l('AddonTimerBarRenBar').style.width = Math.round(Math.min(Game.shimmerTypes['reindeer'].maxTime - Game.shimmerTypes['reindeer'].minTime, Game.shimmerTypes['reindeer'].maxTime - Game.shimmerTypes['reindeer'].time) * maxWidth / Game.shimmerTypes['reindeer'].maxTime) + 'px';
			l('AddonTimerBarRenTime').textContent = Math.ceil((Game.shimmerTypes['reindeer'].maxTime - Game.shimmerTypes['reindeer'].time) / Game.fps);
			count++;
		}
		else {
		...
		for (var i in Game.buffs) {
			if (Game.buffs[i]) {
				buffCount++;
				Addon['TimerBarBuff' + buffCount].style.display = '';
				l('AddonTimerBarBuff' + buffCount + 'Type').textContent = Game.buffs[i].name;
				var classColor = '';
				...
				l('AddonTimerBarBuff' + buffCount + 'Bar').className = Addon.colorBackPre + classColor;
				l('AddonTimerBarBuff' + buffCount + 'Bar').style.width = Math.round(Game.buffs[i].time * maxWidth / Game.buffs[i].maxTime) + 'px';
				l('AddonTimerBarBuff' + buffCount + 'Time').textContent = Math.ceil(Game.buffs[i].time / Game.fps);
				count++;
				if (buffCount == 2) {
					break;
				}
		...
		/*if (Game.frenzy > 0) {
			...
			if (Game.frenzyPower == 7) {
				l('AddonTimerBarBuff1Type').textContent = 'Frenzy';
				l('AddonTimerBarBuff1Bar').className = Addon.colorBackPre + Addon.colorYellow;
			}
			else if (Game.frenzyPower == 0.5) {
				l('AddonTimerBarBuff1Type').textContent = 'Clot';
				l('AddonTimerBarBuff1Bar').className = Addon.colorBackPre + Addon.colorRed;
			}
			else if (Game.frenzyPower == 15) {
				l('AddonTimerBarBuff1Type').textContent = 'Dragon Harvest';
				l('AddonTimerBarBuff1Bar').className = Addon.colorBackPre + Addon.colorBrown;
			}
			else {
				l('AddonTimerBarBuff1Type').textContent = 'Blood Frenzy';
				l('AddonTimerBarBuff1Bar').className = Addon.colorBackPre + Addon.colorGreen;
			}
			l('AddonTimerBarBuff1Bar').style.width = Math.round(Game.frenzy * maxWidth / Game.frenzyMax) + 'px';
			l('AddonTimerBarBuff1Time').textContent = Math.ceil(Game.frenzy / Game.fps);
			count++;
		}
		else {
			Addon.TimerBarBuff1.style.display = 'none';
		...
		if (Game.clickFrenzy > 0) {
			...
			if (Game.clickFrenzyPower == 777) {
				l('AddonTimerBarBuff2Type').textContent = 'Click Frenzy';
				l('AddonTimerBarBuff2Bar').className = Addon.colorBackPre + Addon.colorBlue;
			}
			else {
				l('AddonTimerBarBuff2Type').textContent = 'Dragonflight';
				l('AddonTimerBarBuff2Bar').className = Addon.colorBackPre + Addon.colorPink;
			}
			l('AddonTimerBarBuff2Bar').style.width = Math.round(Game.clickFrenzy * maxWidth / Game.clickFrenzyMax) + 'px';
			l('AddonTimerBarBuff2Time').textContent = Math.ceil(Game.clickFrenzy / Game.fps);
			count++;
		}
		else {
			Addon.TimerBarBuff2.style.display = 'none';
...
Addon.UpdateBuildings = function() {
	if (Addon.BuildColor == 1 && Game.buyMode == 1) {
		...
		for (var i in Addon[target]) {
			l('productPrice' + Game.Objects[i].id).style.color = Addon.Colors[Addon[target][i].color];
		}
	}
	else {
		for (var i in Addon.Objects) {
...
Addon.CreateUpgradeBar = function() {
	...
	Addon.UpgradeBar.id = 'AddonUpgradeBar';
	...
	Addon.UpgradeBar.appendChild(upgradeNumber('AddonUpgradeBarBlue', Addon.colorBlue));
	Addon.UpgradeBar.appendChild(upgradeNumber('AddonUpgradeBarGreen', Addon.colorGreen));
	Addon.UpgradeBar.appendChild(upgradeNumber('AddonUpgradeBarYellow', Addon.colorYellow));
	Addon.UpgradeBar.appendChild(upgradeNumber('AddonUpgradeBarOrange', Addon.colorOrange));
	Addon.UpgradeBar.appendChild(upgradeNumber('AddonUpgradeBarRed', Addon.colorRed));
	Addon.UpgradeBar.appendChild(upgradeNumber('AddonUpgradeBarPurple', Addon.colorPurple));
	Addon.UpgradeBar.appendChild(upgradeNumber('AddonUpgradeBarGray', Addon.colorGray));
...
Addon.UpdateUpgrades = function() {
	if (Addon.UpBarColor == 1) {
		...
		l('AddonUpgradeBarBlue').textContent = blue;
		l('AddonUpgradeBarGreen').textContent = green;
		l('AddonUpgradeBarYellow').textContent = yellow;
		l('AddonUpgradeBarOrange').textContent = orange;
		l('AddonUpgradeBarRed').textContent = red;
		l('AddonUpgradeBarPurple').textContent = purple;
		l('AddonUpgradeBarGray').textContent = gray;
	}
}
...
Addon.CreateWhiteScreen = function() {
	...
	Addon.WhiteScreen.id = 'AddonWhiteScreen';
...
Addon.CreateTooltip = function(placeholder, text, minWidth) {
	Addon[placeholder] = document.createElement('div');
	...
	Addon[placeholder].appendChild(desc);
}
...
Addon.AddMenuPref = function(title) {
	...
	var url = function(config) {
		...
		span.textContent = Addon[config].label + ' ';
		div.appendChild(span);
		var input = document.createElement('input');
		input.id = AddonPrefix + config;
		input.className = 'option';
		input.type = 'text';
		input.value = Addon[config];
		input.style.width = '300px';
		div.appendChild(input);
		div.appendChild(document.createTextNode(' '));
		var a = document.createElement('a');
		a.className = 'option';
		a.onclick = function() {Addon[config] = l(AddonPrefix + config).value;Addon.SaveConfig(Addon);};
		a.textContent = 'Save';
		div.appendChild(a);
		var label = document.createElement('label');
		label.textContent = Addon[config].desc;
	...
	for (var i = 0; i < Addon.colors.length; i++) {
		...
		div.className = 'listing';
		var input = document.createElement('input');
		input.id = AddonPrefix + 'Color' + Addon.colors[i];
		input.className = 'option';
		input.style.width = '65px';
		input.value = Addon.Colors[Addon.colors[i]];
		div.appendChild(input);
		eval('var change = function() {Addon.Colors[\'' + Addon.colors[i] + '\'] = l(AddonPrefix + \'Color\' + \'' + Addon.colors[i] + '\').value; Addon.UpdateColors(); Addon.SaveConfig(Addon);}');
	...
	volText.id = AddonPrefix + volConfig;
	...
	volLabel.textContent = Addon[volConfig].desc;
...
Addon.AddMenuStats = function(title) {
	...
	var listingQuest = function(text, placeholder) {
		...
		span.onmouseover = function() {Game.tooltip.draw(this, escape(Addon[placeholder].innerHTML));};
...
Addon.CreateTooltipWarnCaut = function() {
	...
	Addon.TooltipWarnCaut.appendChild(create('AddonDispTooltipWarn', Addon.colorRed, 'Warning: ', 'Purchase of this item will put you under the number of Cookies required for "Lucky!"', 'AddonDispTooltipWarnText'));
	Addon.TooltipWarnCaut.firstChild.style.marginBottom = '4px';
	Addon.TooltipWarnCaut.appendChild(create('AddonDispTooltipCaut', Addon.colorYellow, 'Caution: ', 'Purchase of this item will put you under the number of Cookies required for "Lucky!" (Frenzy)', 'AddonDispTooltipCautText'));
...
Addon.Tooltip = function(type, name) {
	if (type == 'b') {
		...
		if (Game.buyMode == 1) {
			...
			if (change) {
				l('tooltip').innerHTML = l('tooltip').innerHTML.split(Beautify(Game.Objects[name].getPrice())).join(Beautify(Addon[target][name].price));
			}
		}
...
Addon.UpdateTooltip = function() {
	if (l('tooltipAnchor').style.display != 'none' && l('AddonTooltipArea') != null) {
		...
		if (Addon.tooltipType == 'b') {
			...
			if (Game.buyMode == 1 && Game.buyBulk == 10) {
				target = 'Objects10';
				price = Addon[target][Addon.tooltipName].price;
			}
			...
			bonus = Addon[target][Addon.tooltipName].bonus;
			if (Addon.Tooltip == 1 && Game.buyMode == 1) {
				l('AddonTooltipBorder').className = Addon.colorTextPre + Addon[target][Addon.tooltipName].color;
				l('AddonTooltipPP').textContent = Beautify(Addon[target][Addon.tooltipName].pp, 2);
				l('AddonTooltipPP').className = Addon.colorTextPre + Addon[target][Addon.tooltipName].color;
			}
		}
		else { // Upgrades
			...
			if (Addon.Tooltip == 1) {
				l('AddonTooltipBorder').className = Addon.colorTextPre + Addon.Upgrades[Game.UpgradesInStore[Addon.tooltipName].name].color;
				l('AddonTooltipPP').textContent = Beautify(Addon.Upgrades[Game.UpgradesInStore[Addon.tooltipName].name].pp, 2);
				l('AddonTooltipPP').className = Addon.colorTextPre + Addon.Upgrades[Game.UpgradesInStore[Addon.tooltipName].name].color;
			}
		}
		if (Addon.Tooltip == 1 && (Addon.tooltipType != 'b' || Game.buyMode == 1)) {
			l('AddonTooltipIncome').textContent = Beautify(bonus, 2);
			
			var increase = Math.round(bonus / Game.cookiesPs * 10000);
			if (isFinite(increase) && increase != 0) {
				l('AddonTooltipIncome').textContent += ' (' + (increase / 100) + '% of income)';
			}
		
			var timeColor = Addon.GetTimeColor(price, (Game.cookies + Addon.GetWrinkConfigBank()), Addon.GetCPS());
			l('AddonTooltipTime').textContent = timeColor.text;
			l('AddonTooltipTime').className = Addon.colorTextPre + timeColor.color;
		}
		...
		if (Addon.ToolWarnCaut == 1) {
			...
			if ((amount < warn || amount < caut) && (Addon.tooltipType != 'b' || Game.buyMode == 1)) {
				...
				if (amount < warn) {
					l('AddonDispTooltipWarn').style.display = '';
					l('AddonDispTooltipWarnText').textContent = Beautify(warn - amount) + ' (' + Addon.FormatTime((warn - amount) / Addon.GetCPS()) + ')';
					l('AddonDispTooltipCaut').style.display = '';
					l('AddonDispTooltipCautText').textContent = Beautify(caut - amount) + ' (' + Addon.FormatTime((caut - amount) / Addon.GetCPS()) + ')';
				}
				else if (amount < caut) {
					l('AddonDispTooltipCaut').style.display = '';
					l('AddonDispTooltipCautText').textContent = Beautify(caut - amount) + ' (' + Addon.FormatTime((caut - amount) / Addon.GetCPS()) + ')';
					l('AddonDispTooltipWarn').style.display = 'none';
				}
				else {
					l('AddonDispTooltipWarn').style.display = 'none';
					l('AddonDispTooltipCaut').style.display = 'none';
				}
			}
			else {
				l('AddonDispTooltipWarn').style.display = 'none';
...
Addon.DrawTooltipWarnCaut = function() {
	if (Addon.ToolWarnCaut == 1) {
		l('AddonDispTooltipWarn').style.opacity = '0';
		l('AddonDispTooltipCaut').style.opacity = '0';
	}
}

Addon.UpdateTooltipWarnCaut = function() {
	if (Addon.ToolWarnCaut == 1 && l('tooltipAnchor').style.display != 'none' && l('AddonTooltipArea') != null) {
		l('AddonDispTooltipWarn').style.opacity = '1';
		l('AddonDispTooltipCaut').style.opacity = '1';
	}
}
...
Addon.UpdateWrinklerTooltip = function() {
	if (Addon.ToolWrink == 1 && l('AddonTooltipWrinkler') != null) {
		...
		sucked *= toSuck;
		if (Game.Has('Wrinklerspawn')) sucked *= 1.05;
		l('AddonTooltipWrinkler').textContent = Beautify(sucked);
	}
}
...
Addon.colorTextPre = 'AddonText';
Addon.colorBackPre = 'AddonBack';
Addon.colorBorderPre = 'AddonBorder';
...
Addon.ReplaceNative = function()
 {
	...
	eval('Addon.LogiAddonod = ' + Game.Logic.toString().split('document.title').join('Addon.Title'));	
	Game.Logic = function() {
		Addon.LogiAddonod();
...
AddonDefault = {BotBar: 1, TimerBar: 1, TimerBarPos: 0, BuildColor: 1, BulkBuildColor: 0, UpBarColor: 1, CalcWrink: 1, CPSMode: 1, AvgCPSHist: 2, AvgClicksHist: 2, ToolWarnCautBon: 0, Flash: 1, Sound: 1,  Volume: 100, GCSoundURL: 'http://freesound.org/data/previews/66/66717_931655-lq.mp3', SeaSoundURL: 'http://www.freesound.org/data/previews/121/121099_2193266-lq.mp3', GCTimer: 1, Title: 1, Favicon: 1, Tooltip: 1, TooltipAmor: 0, ToolWarnCaut: 1, ToolWarnCautPos: 1, ToolWrink: 1, Stats: 1, UpStats: 1, TimeFormat: 0, SayTime: 1, Scale: 2, StatsPref: {Lucky: 1, Chain: 1, Prestige: 1, Wrink: 1, Sea: 1, Misc: 1}, Colors : {Blue: '#4bb8f0', Green: '#00ff00', Yellow: '#ffff00', Orange: '#ff7f00', Red: '#ff0000', Purple: '#ff00ff', Gray: '#b3b3b3', Pink: '#ff1493', Brown: '#8b4513'}};
AddonPrefix = 'AddonConfig';
...
eval('Addon.HasAchiev = ' + Game.HasAchiev.toString().split('Game').join('Addon'));

eval('Addon.GetHeavenlyMultiplier = ' + Game.GetHeavenlyMultiplier.toString().split('Game').join('Addon'));
...
Addon.BuyBuildings = function(amount, target) {	
	...
	for (var i in Game.Objects) {
		...
		Addon[target][i] = {};
		Addon[target][i].bonus = Addon.cookiesPs - Game.cookiesPs;
		if (amount != 1) {
			Addon.DoRemakeBuildPrices = 1;
		}
	}
