...
Addon.Disp.AddMenuPref = function(title) {
...
Addon.Disp.AddMenuStats = function(title) {
...
Addon.Disp.AddMenu = function() {
	var title = function() {
		var div = document.createElement('div');
		div.className = 'title ' + Addon.Disp.colorTextPre + Addon.Disp.colorBlue;
		div.textContent = 'Cookie Monster Goodies';
		return div;
	}
	
	if (Game.onMenu == 'prefs') {
		Addon.Disp.AddMenuPref(title);
	}
	else if (Addon.Config.Stats == 1 && Game.onMenu == 'stats') {
		Addon.Disp.AddMenuStats(title);
	}
}
